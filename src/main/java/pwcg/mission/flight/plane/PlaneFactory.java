...
public class PlaneFactory
{
    ...
    List<Plane> planesForFlight = new ArrayList<>();

    public PlaneFactory(Campaign campaign, Squadron squadron, Flight flight)
    {
    ...
    public List<Plane> createPlanesForFlight(int numPlanes) throws PWCGException
    {
    ...
    {        
        for (int numInFormation = 0; numInFormation < planesTypesForFlight.size(); ++numInFormation)
        {
            ...
            Plane plane = PlaneTypeFactory.createPlaneByPlaneType(planeType, squadron.getCountry(), pilot);
	...
	private void initializePlaneParameters() throws PWCGException
	{
        ...
        for (Plane plane : planesForFlight)
        {
            ++numInFormation;
	...
	private void setPlaceInFormation(int numInFormation, Plane aiPlane)
	{
		if (flight.isVirtual())
		{
	...
	private void setPlaneDescription(Plane plane) throws PWCGException
	{
	    plane.setDesc(plane.getPilot().getNameAndRank());
	}

	private void setAiSkillLevelForPlane(Plane plane) throws PWCGException
	{
    ...
    private AiSkillLevel assignAiSkillLevel(Plane plane) throws PWCGException
    {
