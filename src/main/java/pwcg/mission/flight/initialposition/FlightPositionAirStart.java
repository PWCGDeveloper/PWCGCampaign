...
public class FlightPositionAirStart
{
    ...
    {
        if (flight.getFlightData().getWaypointPackage().getAllWaypoints().size() < 2)
        {
            throw new PWCGException("Flight generated with fewer than two waypoints : " + flight.getFlightData().getFlightInformation().getFlightType());
        }
        
        Coordinate firstWaypointPosition = flight.getFlightData().getWaypointPackage().getAllWaypoints().get(0).getPosition();
        Coordinate secondWaypointPosition = flight.getFlightData().getWaypointPackage().getAllWaypoints().get(1).getPosition();
        ...
        startCoordinate.setYPos(WaypointType.getAltitudeForWaypointType(WaypointType.AIR_START_WAYPOINT, flight.getFlightData().getFlightInformation().getAltitude()));
    ...
    {
        double startOrientation = MathUtils.calcAngle(startCoordinate, flight.getFlightData().getFlightInformation().getTargetPosition());
        int initialAltitude = WaypointType.getAltitudeForWaypointType(WaypointType.AIR_START_WAYPOINT, flight.getFlightData().getFlightInformation().getAltitude());
        ...
        for (PlaneMcu plane : flight.getFlightData().getFlightPlanes().getPlanes())
        {
            ...
            flight.getFlightData().getFlightPlanes().setPlanePosition(plane.getLinkTrId(), planeCoords, new Orientation(startOrientation), FlightStartPosition.START_IN_AIR);
