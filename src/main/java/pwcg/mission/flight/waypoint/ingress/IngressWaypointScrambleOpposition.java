...
public class IngressWaypointScrambleOpposition implements IIngressWaypoint
{
    ...
    public McuWaypoint createIngressWaypoint() throws PWCGException  
    {
        ...
        FrontLinePoint nearbyFrontPoint = frontLinesForMap.findCloseFrontPositionForSide(flight.getPosition(), 20000, flight.getSquadron().getCountry().getSide());
        ...
        double ingressAngle = MathUtils.calcAngle(nearbyFrontPosition, flight.getTargetPosition());
        Coordinate ingressCoords = MathUtils.calcNextCoord(flight.getTargetPosition(), ingressAngle, attackIngressDistance);
