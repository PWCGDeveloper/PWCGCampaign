...
public class IngressWaypointNearFront implements IIngressWaypoint
{
    ...
    private Coordinate getBestIngressPositionForCommonAIFlights() throws PWCGException 
    {
        ...
        Coordinate closestFrontLinesToTarget = frontLinesForMap.findClosestFrontCoordinateForSide(flight.getTargetPosition(), flight.getSquadron().determineSide());
    ...
    {
        FrontLinesForMap frontLinesForMap =  PWCGContext.getInstance().getCurrentMap().getFrontLinesForMap(campaign.getDate());
        Coordinate closestEnemyFrontLinesToTarget = frontLinesForMap.findClosestFrontCoordinateForSide(flight.getTargetPosition(), flight.getSquadron().determineSide().getOppositeSide());
        Coordinate closestFriendlyFrontLinesToTarget = frontLinesForMap.findClosestFrontCoordinateForSide(flight.getTargetPosition(), flight.getSquadron().determineSide());
