...
public class TacticalCodeBritainWWI extends TacticalCode
{
    ...
    {
        if (plane.getType().equals(FCPlaneAttributeMapping.DOLPHIN.getPlaneType()))
        {
            return true;
        }
        else if (plane.getType().contains("spad"))
        {
            return true;
        }
        else
    ...
    private void formWWICode(PlaneMcu plane, List<String> codes) throws PWCGException
    {
        ...
        else if (plane.getSkin().getTacticalCodeType() == TacticalCodeType.CODE_POSITION_TWO)
        {
            ...
            {
                throw new PWCGException("Cannot use British position 2 with an Airco DH4");
            }
        ...
        {
            if (!plane.getType().contains("spad"))
            {
                throw new PWCGException("Only British SPAD suppoorted for position 1 and 2");
            }
