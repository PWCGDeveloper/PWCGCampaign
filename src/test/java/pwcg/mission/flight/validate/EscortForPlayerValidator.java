...
public class EscortForPlayerValidator 
{
    ...
    public EscortForPlayerValidator (Flight playerFlight)
    {
    ...
    private void validateRendezvous()
    {
        ...
        PlaneMCU leadEscortedPlane = playerFlight.getPlanes().get(0);
    ...
    private void validateLinkedEscort()
    {
        for (IUnit unit : playerFlight.getLinkedUnits())
        {
            ...
            {
                assert(unit instanceof EscortForPlayerFlight);
    ...
    {
        assert(escortForPlayerFlight != null);
        assert(escortForPlayerFlight.isVirtual() == false);
        assert(escortForPlayerFlight.isAirStart() == true);
        assert(escortForPlayerFlight.isPlayerFlight() == false);
    }

    private void validateEscortAltitude()
    {
        for (PlaneMCU escortPlane : escortForPlayerFlight.getPlanes())
        {
	...
	private void validateWaypointLinkage() throws PWCGException 
	{
        ...
        assert(isIndexInTargetList(playerFlight.getLeadPlane().getEntity().getIndex(), cover.getTargets()));
